...
// Define types for database nodes and edges
interface DbNode {
  id: string;
  type: "TRIGGER" | "ACTION" | "CONDITION" | string;
  label?: string;
  positionX: number;
  positionY: number;
  config?: {
...
export function transformDbNodesToReactFlow(dbNodes: DbNode[]): ReactFlowNode[] {
...
export function transformDbEdgesToReactFlow(dbEdges: DbEdge[]): ReactFlowEdge[] {
